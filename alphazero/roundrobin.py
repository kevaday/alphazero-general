    ...
    for i in range(model_count - 1):
        for j in range(i + 1, model_count):
            ...
            if args.numMCTSSims <= 0:
                p1 = NNPlayer(g, nnet1, args.arenaTemp, args.tempThreshold).play
            else:
                p1 = MCTSPlayer(g, nnet1, args).play

            if file2.name != 'random':
                nnet2.load_checkpoint(folder='roundrobin', filename=file2.name)
                if args.numMCTSSims <= 0:
                    p2 = NNPlayer(g, nnet2, args.arenaTemp,
                                  args.tempThreshold).play
                else:
                    p2 = MCTSPlayer(g, nnet2, args)
            else:
                p2 = RandomPlayer(g).play
            arena = Arena([p1, p2], g, use_batched_mcts=True, args=args)
